myApp.controller('AdminController', ['$scope', '$rootScope', '$firebaseAuth', '$firebaseArray', 'Authentication',         
  function ($scope, $rootScope, $firebaseAuth, $firebaseArray, Authentication) {
      ...
      $scope.toggleAssociate = function(item){
          ...
          if( item.associate == 'on' ) {
              ...
              $scope.staffers.forEach(function(member) {
                  if(member.regUID === item.regUser) {
                      alert('staffmember profile deactivated!');
                      member.profile[0] = 'off';
                      $scope.staffers.$save(member);
                  };
              });
          } else {
              item.associate = 'on';
